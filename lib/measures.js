...
export function lastMeasureWithinLastDays(cli, measures, days=1){
    ...
    if(null == days || isNaN(days) || 0 > days){
        log.warn("WARN: days are invalid: " + days)
        return null
    }
    log.verbose("checking if we had measures yesterday (" + getTodayDate(cli, -1) + ")...")
    ...
    if(undefined === measures[getTodayDate(cli, -1)]){
        log.silly("found NO measures for date " + getTodayDate(cli, -1) + "")
        log.verbose("\tthere was no measure yesterday... continuing\n")
        return false
    }
    log.verbose("\tfound " + Object.keys(measures[getTodayDate(cli, -1)]).length + " measures at date (" + getTodayDate(cli, -1) + "). ")
    log.debug("\t  keys (times of measurement) are: " + Object.keys(measures[getTodayDate(cli, -1)]).toString())
    return true
}
...
export function longerBreakRequired(cli, measures){
    ...
    if(undefined === measures[getTodayDate(cli)]){
        log.verbose("found NO measures for today (" + getTodayDate(cli) + ") ")
        log.silly("measures received: " + JSON.stringify(measures, null, 2))
        return false
    }
    ...
    log.verbose("checking if we had 5 measures today already and need to take a break of at least 3 hours... (" + getTodayDate(cli) + ")")
    if(5 !== Object.keys(measures[getTodayDate(cli)]).length){
        log.verbose("\twith " + Object.keys(measures[getTodayDate(cli)]).length + " measures, we don't need to take a longer break... continuing.")
        return false
    }
    ...
    log.debug("parsed str " + parseDateStr + " to " + 
              "\n\tdate param: " + cli.flags.date +
              "\n\ttime param: " + cli.flags.time +
              "\n\tgenerated date: " + getTodayDate(cli) +
              "\n\tgenerated time: " + getTodayTime(cli))
    ...
    for(i = 0; i < 4; i++){
        ...
        for(const timeMeasure of Object.keys(measures[getTodayDate(cli)])){
            ...
            if(timeMeasure.startsWith(now.getHours())){
                log.silly(timeMeasure + " start with current hour: " + now.getHours())
                log.debug("\twe are between 5th and 6th measure and have to take a break of at least 3 hours (now: " + (now.getHours() + i) + "h).")
                log.debug("\tfound a measure at " + now.getHours() + "h. will further pause until " + (now.getHours() + 4) + "h")
                return true
            }
        }
        now.setHours(now.getHours() - 1)
    }
    log.verbose("\twe already had five measures today, however, the last check was before " + (now.getHours() + 1) + "h. we can continue.")
    return false
}
