...
export function longerBreakRequired(cli, measures){
    ...
    if(cli.flags.verbose){
        console.log("checking if we had 5 measures today already and need to take a break of at least 3 hours... (" + getTodayDate(cli) + ")")
    }
    if(5 !== Object.keys(measures[getTodayDate(cli)]).length){
        if(cli.flags.verbose){
            console.log("\twith " + Object.keys(measures[getTodayDate(cli)]).length + " measures, we don't need to take a longer break... continuing.")
        }
        return false
    }
