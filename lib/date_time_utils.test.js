...
test('happy path with getTodayTime', () => {
    ...
    let config = cli()
    config.debug = true
    expect(getParseableTodayTime(config)).toBe(now.getHours() + ":" + now.getMinutes())
    expect(getTodayTime(config)).toBe(now.getHours().toString() + now.getMinutes().toString())
...
test('happy path with getTodayDate', () => {
    ...
    let config = cli()
    config.debug = true
    expect(getParseableTodayDate(config)).toBe(now.getFullYear() + "-" + (1 + now.getMonth()) + "-" + (now.getDate() < 10 ? "0" + now.getDate() : "" + now.getDate()))
    expect(getTodayDate(config)).toBe(now.getFullYear().toString() + (1 + now.getMonth()).toString() + (now.getDate() < 10 ? "0" + now.getDate() : "" + now.getDate()))
