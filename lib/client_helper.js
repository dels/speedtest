...
function checkConfig(cli){
    ...
    for(const param of parameters){
        ...
        if(process.env[param] && null == cli.flags[paramCamelCase]){
            log.debug("we found no value in params for " + param + " but in .env - using .env value")
            cli.flags[paramCamelCase] = process.env[param]
        }
        if(process.env[param] && cli.flags[paramCamelCase]){
            log.debug(param + ": found value in params and in .env - using parameter value")
        }
        
        if(isNaN(cli.flags[paramCamelCase]) || 0 > cli.flags[paramCamelCase]){
            log.info("\n\n\tHINT: specify " + param + " in .env\n\n")
            let err = new Error()
            err.message = (param + " not defined or invalid: " + cli.flags[paramCamelCase])
            throw err
        }
        ...
        log.debug("setting cli.flags[paramCamelCase] to: " + cli.flags[paramCamelCase])
