...
export function execute(cli){
    ...
    const now = new Date()
    const date = "" + now.getFullYear() + (now.getMonth() + 1) + now.getDate() + "-" + fillWithZero(now.getHours()) + fillWithZero(now.getMinutes())
    ...
    const jsonFileStr = home + "/" + cli.flags.dataDir + "/" + date + ".json"
