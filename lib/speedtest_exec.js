...
import { cliValid } from './lib/client_helper.js'
...
export function execute(cli){
    ...
    if(cli.flags.debug){
        console.debug("speedtest home: " + home)
    }
    const now = new Date()
    ...
    if(false === cli.flags.quiet && (cli.flags.verbose)){
        console.debug("starting speedtest... saving file to " + jsonFileStr)
    }
    exec(bin + " --json > " + jsonFileStr, (error, stdout, stderr) => {
        ...
        if(cli.flags.verbose)
            console.log("measure result saved at " + jsonFileStr)
    })
}
